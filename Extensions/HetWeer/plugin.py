...
class secondmenu(Screen):
	...
	def lijst(self):
		if self.iweertext == 'Neerslag Radars':
			...
			self.options.append((_("Zomerradar NL"),"http://zomerradar.buienradar.nl",'Zomerradar waar schijnt de zon!\nGele kleur is zonnig gebied.'))
			...
			#self.options.append((_("Radar west Europa"),"http://europa.buienradar.nl/",'Live radar voor west Europa, voor als je op de fiets naar Frankrijk wil.'))
			self.options.append((_("Radar Europa"),"http://sat24.com/?ir=true&ra=true&li=false",'Live radar voor Europa, voor als je op de fiets naar Spanje wil.'))
			...
			#Temperatuur
			self.options = []
			self.options.append((_("Knmi"),"http://www.onweer-online.nl/images/maps/tempknmi.png",'knmi actuele temperatuur kaart \n\nKaart is stilstaand.'))
			...
			self.options.append((_("Weerplaza grond"),"http://www.onweer-online.nl/images/maps/temperatuur_10_cm.png",'Temperatuur kaart op 10cm hoogte van weerplaze \n\nKaart is stilstaand.'))
		...
		try:
			if self.iweertext == 'Pluimgrafieken':
				...
				if self.keuzetekst == "Buienradar NL":
					getPage(self.url).addCallback(self.done,'/radar','nl').addErrback(self.downloadError)
				...
				elif self.keuzetekst == "Radar west Europa":
					getPage(self.url).addCallback(self.done,'h.aspx?','eu', ).addErrback(self.downloadError)
